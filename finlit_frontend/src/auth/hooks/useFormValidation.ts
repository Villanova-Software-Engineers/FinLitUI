...
export const useFormValidation = () => {
  ...
  const validateOrganizationCode = useCallback((code: string): string => {
    if (!code) return 'Organization code is required';
    if (!/^[a-zA-Z0-9]+$/.test(code)) return 'Organization code must be alphanumeric';
    if (code.length < 3) return 'Organization code is too short';
    return '';
  }, []);
