    ...
    return (
      <div className="min-h-screen p-6 pt-20 flex items-center justify-center bg-gradient-to-br from-red-50 to-orange-100">
        ...
        >
          ...
          <p className="text-xl text-gray-600 mb-6">
            You scored <span className="font-bold text-orange-600">{score}/{questions.length}</span> ({(score/questions.length*100).toFixed(0)}%)
          </p>
  ...
  const handleNextQuestion = () => {
    ...
    if (currentQuestion < shuffledQuestions.length - 1) {
      ...
      if ((finalScore / shuffledQuestions.length) * 100 >= 80) {
        saveScore(MODULES.FINANCIAL_SAFETY.id, (finalScore / shuffledQuestions.length) * 100);
      }
    }
  };
