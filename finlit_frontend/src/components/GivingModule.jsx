      ...
      <div className="min-h-screen p-6 pt-20 flex items-center justify-center bg-gradient-to-br from-pink-50 to-rose-100">
        ...
        >
          ...
          <p className="text-xl text-gray-600 mb-6">
            You scored <span className="font-bold text-pink-600">{score}/{quizQuestions.length}</span> ({(score/quizQuestions.length*100).toFixed(0)}%)
          </p>
  ...
  const handleNextQuestion = () => {
    ...
    if (currentQuestion < quizQuestions.length - 1) {
      ...
      if ((finalScore / quizQuestions.length) * 100 >= 80) {
        saveScore(MODULES.GIVING.id, (finalScore / quizQuestions.length) * 100);
      }
    }
  };
